...
export const neo4jSwipe = async (id: string, swipe: string, userId: string) => {
  ...
  if(type === 'LIKE'){
    const result = await driver.executeQuery(`MATCH (cu:User {applicationId : $id}),(ou: User {applicationId : $userId}) WHERE (ou)-[:LIKE]->(cu) RETURN ou as match`,
    {
        id,
        userId
    }
  )
  const matches = result.records.map((rec)=>rec.get('match').properties);
...
export const getMatches = async (currentUserId : string) => {
    const result = await driver.executeQuery(`MATCH (cu:User {applicationId : $id})-[:LIKE]-(ou : User)-[:LIKE]->(cu) RETURN ou as match`,
        {
            id : currentUserId,
